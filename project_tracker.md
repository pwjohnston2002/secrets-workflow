...
|---|---|---|---|---|
| `.github/workflows/terraform-ephemeral.yml` | Reusable workflow | Implemented | Contract surface now declares inputs plus outputs (`plan_artifact_path`, `apply_executed`, `destroy_executed`, `destroy_succeeded`); behavior guarantees still need release-level stabilization | Keep in inventory while contract semantics are finalized |
